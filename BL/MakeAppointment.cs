    ...
    public class MakeAppointment
    {
        ...
        private static TimeSpan? lookForAvailableTurn(ActivityTimeDTO activityTime, ref bool pushFlag, TimeSpan timeToLookFor)
        {
            ...
            for (hour = timeToLookFor; numOfStandbyTime < maxStandbyTime || hour < activityTime.EndTime; hour = hour.Add(ts), numOfStandbyTime++)
            {
                ...
                if (index>=line.Count() || line[index].estimatedHour.TimeOfDay >= hour.Add(ts))
                {
                    return hour;
                }
                                                                     ...
                                                                     // בודק תור פנוי גם לפי מספר הקופות
                {
                    ...
                    if (countTurnsForSameHour < activityTime.NumOfWorkers)
                        return hour;
                }
                index++;
            }
        ...
        {
            //todo:??????????????????
            string code = "" + turn.TurnId + turn.activityTime.service.businessId;
            return code;
        }
